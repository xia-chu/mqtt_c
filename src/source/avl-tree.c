...
struct _AVLTreeNode {
	...
	AVLTreeKey key;
	AVLTreeValue value;
    AVLTreeFreeKeyFunc key_free;
    AVLTreeValueKeyFunc value_free;
	int height;
};
...
void free_node_key_value(AVLTreeNode *node){
    if (node->key_free && node->key){
        node->key_free(node->key);
        node->key = NULL;
        node->key_free = NULL;
    }

    if (node->value_free && node->value){
        node->value_free(node->value);
        node->value = NULL;
        node->value_free = NULL;
    }
}
void free_node(AVLTreeNode *node){
    if(!node){
        return;
    }
...
AVLTreeNode *avl_tree_insert(AVLTree *tree, AVLTreeKey key, AVLTreeValue value,
                             AVLTreeFreeKeyFunc freeKey,
                             AVLTreeValueKeyFunc freeValue)
{
	AVLTreeNode **rover;
	...
	while (*rover != NULL) {
		previous_node = *rover;
		int ret = tree->compare_func(key, (*rover)->key);
		if (ret < 0) {
			rover = &((*rover)->children[AVL_TREE_NODE_LEFT]);
		} else if(ret > 0){
            rover = &((*rover)->children[AVL_TREE_NODE_RIGHT]);
		} else {
            free_node_key_value(*rover);
            new_node = *rover;
            new_node->key = key;
            new_node->key_free = freeKey;
            new_node->value_free = freeValue;
...
static void avl_tree_to_array_add_subtree_node(AVLTreeNode *subtree,
                                               AVLTreeNode **array,
                                               int *index)
{
    if (subtree == NULL) {
    ...
    avl_tree_to_array_add_subtree_node(subtree->children[AVL_TREE_NODE_LEFT],array, index);
...
AVLTreeNode **avl_tree_to_array_node(AVLTree *tree){
    ...
    avl_tree_to_array_add_subtree_node(tree->root_node, array, &index);
